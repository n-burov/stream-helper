...
// ============================================================
//  РУЧНАЯ УСТАНОВКА СТАТУСА (для отладки)
// ============================================================

async function setConnectedStatus() {
    await redis.set('twitch_webhook_registered', 'true');
    console.log('✅ Статус подключения установлен вручную');
    return true;
}
...
export default async function handler(req, res) {
    ...
    if (req.method === 'GET') {
        ...
        // Ручная установка статуса через GET (для отладки)
    ...
    if (req.method === 'POST') {
        try {
            ...
            // Ручная установка статуса через POST
